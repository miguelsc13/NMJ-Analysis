...
% Compare two methods of getting frame rate against each other and throw
% error if they're different
% if ~(abs(info.frameRate - frameRateCheck) < 1)
%     error(['Frame Rate Incompatible: info.frameRate = ' ...
%         num2str(info.frameRate) ' but frameRateCheck = ' ...
%         num2str(frameRateCheck)]);
% end
...
for frameIdx = 1 : info.numFrames
    ...
    % update waitbar
    waitbar(frameIdx/info.numFrames, h);
end
close(h)
